...
The last component of the validation application is the client frontend, which is an AngularJS~\cite{AngularJS} single page application (SPA).
...
\textit{User layouts} page (see Fig.~\ref{fig:layouts}) is used to perform fast visual validation of Geant4.  One can use one of the existing templates or create and upload their own one. % It is possible to compare different versions between themselves or against experimental data.
